document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelector(".site-nav__menu")?.addEventListener("click", (event) => { if (event.target !== button && !["atp-filters-btn", "atp-similar-btn"].includes(event.target.id)) { view.hidden = true; button.classList.remove("is-active"); button.setAttribute("aria-pressed", "false"); } });
});
